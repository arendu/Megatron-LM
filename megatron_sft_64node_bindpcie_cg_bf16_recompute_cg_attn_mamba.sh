#!/bin/bash
#
# 64-node SFT — megaron_sft.sh 기반 + 성공한 proxy 설정 반영
#   (proxy-sft-hybridep-bindpcie-cg-bf16-recompute-cg-attn-mamba)
#
# 반영 사항:
#   - bindpcie 런치, NCCL_GRAPH_REGISTER=0 (CUDA graph + expandable_segments)
#   - BF16 only (MXFP8 제거)
#   - --cuda-graph-scope mamba attn (moe_router 제외; recompute moe와 호환)
#   - --log-timers-to-tensorboard
#
# 64-node: nodes=64, segment=16, EP=128, CP=16, experts=512, seq=262144
# Logs/ckpts: sft-runs/ultra-sft-64node-bindpcie-cg-bf16-recompute-cg-attn-mamba/
...
#SBATCH --mem=0
#SBATCH --segment=16
#SBATCH --job-name=ultra-sft-64node-bindpcie-cg-bf16-recompute-cg-attn-mamba
...
################################################################
### HybridEP / MNNVL (64-node: segment=16 => 64 ranks per NVLink domain; EP 64 fits in one domain)
################################################################
...
################################################################
### General (NCCL_GRAPH_REGISTER=0 for CUDA graph + expandable_segments)
################################################################
...
# 64-node: pretrained base (megaron_sft.sh와 동일)
...
# te_patches intercepts TE's context_parallel.py to fix GPU->CPU sync inside CUDA graph capture
...
srun -l \
     ...
     --container-env=NUM_OF_HYBRID_EP_RANKS_PER_NVLINK_DOMAIN,USE_MNNVL,NCCL_GRAPH_REGISTER,UCX_MEM_MMAP_HOOK_MODE,UCX_MEM_CUDA_HOOK_MODE,UCX_MEM_MALLOC_HOOKS,UCX_ERROR_SIGNALS,NVTE_CPU_OFFLOAD_V1,PYTHONPATH,SLURM_LOCALID \
