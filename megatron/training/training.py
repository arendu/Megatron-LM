...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True, config=None, pg_collection=None):
    ...
    # This must happen after Float16Module wrapping (so param names have "module." prefix)
    # but before DDP wrapping (which builds gradient buffers only for trainable params).
    if getattr(args, 'freeze_base_model_for_mtp', False):
        ...
        for model_module in model:
            ...
            for name, module in model_module.named_modules():
                if hasattr(module, 'expert_bias') and 'mtp.' not in name:
                    module._freeze_expert_bias = True
        print_rank_0(
